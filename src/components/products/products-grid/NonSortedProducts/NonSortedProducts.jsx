...
function NonSortedProducts(props) {
    ...
    useEffect(() => {
        const getProducts = () => {
            const pages  = Math.floor(props.data.products.length / recordsPerPage);
            const maxIndex = props.data.products.length % recordsPerPage === 0 ? pages : pages + 1;
        ...
        <Row className="non-sorted-products" ref={listRef}>
            {
                ...
                props.data.productName === '' ? data.map(product => props.data.filterByCategory(product, props.data.category)) : props.data.products.map(product => product.name.toLowerCase().includes(props.data.productName.toLowerCase()) ? props.data.filterByCategory(product, props.data.category) : null)
                // data.map((product) => {
